...
class ShipReportWizard(models.TransientModel):
    ...
    def _get_data(self):
        ...
        if self.customer_id:
            domain.append(('hbl_customer_id', '=', self.customer_id.id))

        if self.salesperson_id:
            domain.append(('employee_id', '=', self.salesperson_id.id))
    ...
    def action_print_pdf(self):
        return self.env.ref(
            'shipping_report.ship_report_pdf_action'
        ).report_action(self)

    def action_export_xlsx(self):
        return self.env.ref(
            'shipping_report.ship_report_xlsx_action'
        ).report_action(self)
